from secure_rag.agent.guardrails import filter_injected_chunks, heuristic_is_indirect_injection
...
def test_fail_closed_on_missing_permission_and_tool():
    ...
    assert result["contexts"] == [] or "No relevant context" in result["answer"] or "CANARY" not in result["answer"]
